...
const TutorialContent = () => {
  const GuideData = useSelector((state) => state.Guide.guide);

  let names, values;

  if (GuideData.attributeNames && GuideData.attributeNames) {
    names = GuideData.attributeNames.split(',');
    values = GuideData.attributeExplanation.split(',');
  }
  ...
  return (
    <Box
      ...
      <div
        ...
        <p className='description'>{GuideData.content}</p>
        <table className=''>
          <tr>
            <th>Tag</th>
            <th>Description</th>
        ...
        <div className='textarea' name='description' rows='12' col='550'>
          {GuideData.example ? (
            GuideData.example.split(',').map((ex) => {
              return <p>{ex}</p>;
            })
          ) : (
            <p>There is no example for this tag</p>
